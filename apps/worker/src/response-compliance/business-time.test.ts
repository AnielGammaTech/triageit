...
describe("response business time", () => {
  it("uses the 8:00 AM to 5:15 PM Eastern weekday window", () => {
    expect(isResponseBusinessTime(new Date("2026-07-14T12:00:00Z"))).toBe(true); // 8:00 ET
    expect(isResponseBusinessTime(new Date("2026-07-14T21:14:00Z"))).toBe(true); // 5:14 ET
    expect(isResponseBusinessTime(new Date("2026-07-14T21:15:00Z"))).toBe(false);
    expect(isResponseBusinessTime(new Date("2026-07-18T14:00:00Z"))).toBe(false);
  });

  it("pauses a deadline overnight", () => {
    expect(addResponseBusinessMinutes(new Date("2026-07-14T21:00:00Z"), 30).toISOString())
      .toBe("2026-07-15T12:15:00.000Z");
  });

  it("pauses a deadline over the weekend", () => {
    expect(addResponseBusinessMinutes(new Date("2026-07-17T21:00:00Z"), 30).toISOString())
