...
export interface AgentChatConfig {
  ...
  readonly conversationsEndpoint: string;
  readonly skillsEndpoint: string;
  ...
  readonly skillLearnHint: string;
}

interface AgentChatProps {
  readonly config: AgentChatConfig;
  readonly ticketContext?: TicketContext;
}

// ── Component ───────────────────────────────────────────────────────────

export function AgentChat({ config, ticketContext }: AgentChatProps) {
  ...
  const [agentAvatar, setAgentAvatar] = useState(config.avatarUrl);
  ...
  const loadConversations = useCallback(async () => {
    const res = await fetch(config.conversationsEndpoint);
    if (res.ok) {
  ...
  useEffect(() => {
    ...
    // Only Prison Mike uses branding avatar
    if (config.name === "Prison Mike") loadAvatar();
  }, [config.name]);
  ...
  }, [config.messagesTable]);

  // Load skills
  const loadSkills = useCallback(async () => {
    const res = await fetch(config.skillsEndpoint);
    if (res.ok) {
      const data = await res.json();
  ...
  const deleteConversation = useCallback(async (convId: string) => {
    await fetch(`${config.conversationsEndpoint}?id=${convId}`, { method: "DELETE" });
    if (activeConversationId === convId) startNewConversation();
    loadConversations();
  }, [activeConversationId, startNewConversation, loadConversations, config.conversationsEndpoint]);

  const deleteSkill = useCallback(async (skillId: string) => {
    await fetch(`${config.skillsEndpoint}?id=${skillId}`, { method: "DELETE" });
    loadSkills();
  }, [loadSkills, config.skillsEndpoint]);
      ...
      while (true) {
        ...
        for (const line of lines) {
          ...
          if (json.done) {
            ...
            setMessages((prev) => [...prev, { id: `assistant-${Date.now()}`, role: "assistant", content: fullText, meta }]);
            setStreamingText("");
            loadConversations();
            if (fullText.includes("[SKILL_LEARNED:")) loadSkills();
          }

          if (json.error) console.error("Stream error:", json.error);
        }
      }
    } catch (err) {
      if (err instanceof DOMException && err.name === "AbortError") {
        if (fullText) {
          setMessages((prev) => [...prev, { id: `assistant-${Date.now()}`, role: "assistant", content: fullText + "\n\n*[Stopped]*" }]);
        }
        setStreamingText("");
      } else {
  ...
  }, [input, streaming, activeConversationId, ticketContext, loadConversations, loadSkills, config.apiEndpoint]);
    ...
    <div className="flex h-[calc(100vh-3.5rem)] overflow-hidden">
      ...
      <div className="flex flex-1 flex-col min-w-0">
        ...
        <div className="flex items-center gap-3 border-b border-white/[0.06] px-4 py-3">
          ...
          <Image unoptimized src={agentAvatar} alt={config.name} width={32} height={32} className="h-8 w-8 rounded-full object-cover" />
          <div>
            <h2 className="text-sm font-semibold text-white">{config.name}</h2>
            <p className="text-[11px] text-white/40">{config.subtitle}</p>
          </div>
          <div className="ml-auto flex items-center gap-3">
            {sessionTokens > 0 && (
              <div className="flex items-center gap-2 rounded-lg bg-white/[0.04] px-3 py-1.5">
                ...
                <span className="text-[11px] font-medium text-amber-400/70">${sessionCost < 0.01 ? sessionCost.toFixed(4) : sessionCost.toFixed(2)}</span>
              </div>
            )}
            {ticketContext?.halo_id && (
              <span className="rounded-md bg-white/[0.06] px-2 py-1 text-xs text-white/50">Ticket #{ticketContext.halo_id}</span>
            )}
          </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto px-4 py-4 space-y-4">
          {messages.length === 0 && !streaming && (
            <div className="flex flex-col items-center justify-center h-full text-center">
              <Image unoptimized src={agentAvatar} alt={config.name} width={64} height={64} className="h-16 w-16 rounded-full object-cover mb-4" />
              <h3 className="text-lg font-semibold text-white/80 mb-1">{config.emptyTitle}</h3>
              <p className="text-sm text-white/40 max-w-md">{config.emptyDescription}</p>
              <div className="mt-6 flex flex-wrap justify-center gap-2">
                {config.suggestions.map((suggestion) => (
                  <button key={suggestion} onClick={() => { setInput(suggestion); inputRef.current?.focus(); }} className="rounded-lg border border-white/[0.08] bg-white/[0.03] px-3 py-2 text-xs text-white/50 hover:text-white/70 hover:bg-white/[0.06] transition-colors text-left">
                    {suggestion}
                  </button>
                ))}
              </div>
          ...
          {messages.map((msg) => (
            <div key={msg.id} className={cn("flex gap-3", msg.role === "user" && "justify-end")}>
              {msg.role === "assistant" && <Image unoptimized src={agentAvatar} alt={config.name} width={28} height={28} className="h-7 w-7 shrink-0 rounded-full object-cover mt-0.5" />}
              <div className="max-w-[75%]">
                <div className={cn("rounded-xl px-4 py-2.5 text-sm leading-relaxed", msg.role === "user" ? `text-white` : "bg-white/[0.06] text-white/90")} style={msg.role === "user" ? { backgroundColor: config.accentColor } : undefined}>
                ...
                {msg.role === "assistant" && msg.meta && (
                  <div className="mt-1 flex items-center gap-2 text-[10px] text-white/25 px-1">
                    ...
                    {msg.meta.input_tokens != null && msg.meta.output_tokens != null && <span>· {(msg.meta.input_tokens + msg.meta.output_tokens).toLocaleString()} tokens</span>}
                    {msg.meta.cost_usd != null && <span>· ${msg.meta.cost_usd < 0.01 ? msg.meta.cost_usd.toFixed(4) : msg.meta.cost_usd.toFixed(2)}</span>}
                  </div>
                )}
              </div>
            </div>
          ...
          {streaming && streamingText && (
            <div className="flex gap-3">
              <Image unoptimized src={agentAvatar} alt={config.name} width={28} height={28} className="h-7 w-7 shrink-0 rounded-full object-cover mt-0.5" />
            ...
            <div className="flex gap-3">
              ...
              <div className="rounded-xl bg-white/[0.06] px-4 py-3 min-w-[200px]">
                    ...
                    {activityLog.map((entry, i) => {
                      ...
                      return (
                        <div key={i} className="flex items-center gap-2 text-xs">
                          ...
                          <span className={isCompleted ? "text-white/30 line-through" : "text-white/50"}>{parsed.text}</span>
                          {parsed.worker && !isCompleted && <span className="rounded bg-amber-400/10 px-1.5 py-0.5 text-[10px] text-amber-400/70">{parsed.worker}</span>}
                        </div>
                      );
                    })}
                  </div>
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs text-white/30">{statusText || config.thinkingText}</span>
                </div>
              </div>
            </div>
          )}
        ...
        <div className="border-t border-white/[0.06] p-4">
          <div className="flex items-end gap-3">
            ...
            />
            {streaming ? (
              <button onClick={stopStreaming} className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-white/10 text-white/70 transition-colors hover:bg-white/20 hover:text-white" title="Stop generating">
                <svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor"><rect x="6" y="6" width="12" height="12" rx="2" /></svg>
              </button>
            ) : (
              <button onClick={sendMessage} disabled={!input.trim()} className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl text-white transition-colors disabled:opacity-30 disabled:cursor-not-allowed" style={{ backgroundColor: config.accentColor }}>
    ...
    <div className="space-y-1.5">
      {lines.map((line, i) => {
        ...
        if (line.startsWith("- ") || line.startsWith("* ")) {
          return <div key={i} className="flex gap-2 ml-1"><span className="text-white/30 mt-0.5">•</span><span><InlineFormatted text={line.slice(2)} /></span></div>;
        }
        if (line.match(/^\d+\.\s/)) {
          const numEnd = line.indexOf(". ");
          return <div key={i} className="flex gap-2 ml-1"><span className="text-white/40 font-mono text-xs mt-0.5">{line.slice(0, numEnd + 1)}</span><span><InlineFormatted text={line.slice(numEnd + 2)} /></span></div>;
        }
  ...
  return (
    <>
      {parts.map((part, i) => {
        ...
        if (part.startsWith("`") && part.endsWith("`")) return <code key={i} className="rounded bg-white/10 px-1 py-0.5 text-xs font-mono text-amber-300">{part.slice(1, -1)}</code>;
        if (part.match(/^#\d{4,6}$/)) return <span key={i} className="font-semibold text-amber-400">{part}</span>;
