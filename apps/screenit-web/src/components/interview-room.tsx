...
import { Check, Clock3, FileCheck2, LoaderCircle, LockKeyhole, MessageSquareText, Mic, MicOff, ShieldCheck, Sparkles, Volume2 } from "lucide-react";
...
export function InterviewRoom({ candidate, position }: { readonly candidate: Candidate; readonly position: Position }) {
  ...
  return <main className="relative min-h-screen overflow-hidden bg-[radial-gradient(circle_at_20%_0%,rgba(20,184,166,.09),transparent_28rem),#f4f7f6]">
    <span className="pointer-events-none absolute -right-24 top-32 h-72 w-72 rounded-full border-[40px] border-teal-100/40" />
    <header className="relative z-10 border-b border-white/10 bg-[linear-gradient(110deg,#11251f,#173c34)] px-5 py-4 shadow-[0_12px_28px_-24px_rgba(3,24,20,.9)]"><div className="mx-auto flex max-w-5xl items-center justify-between"><ScreenItLogo /><span className="flex items-center gap-2 text-xs text-slate-300"><LockKeyhole className="h-3.5 w-3.5 text-teal-300" />Private candidate interview</span></div></header>
    <div className="relative z-10 mx-auto max-w-5xl px-5 py-8 lg:py-12">
      {phase === "welcome" && <section className="mx-auto max-w-3xl overflow-hidden rounded-[28px] border border-emerald-950/10 bg-white shadow-[0_30px_80px_-50px_rgba(8,60,51,.6)]">
        <div className="screenit-dot-grid relative isolate overflow-hidden bg-[linear-gradient(125deg,#17342d_0%,#176358_70%,#0f8f83_130%)] p-7 text-white sm:p-9"><span className="screenit-hero-orb pointer-events-none absolute -right-14 -top-20 h-48 w-48 rounded-full border border-white/10 bg-teal-200/10" /><div className="relative z-10"><div className="flex flex-wrap items-center gap-2"><span className="inline-flex items-center gap-1.5 rounded-full border border-teal-100/15 bg-white/[0.07] px-2.5 py-1 text-[10px] font-bold uppercase tracking-[0.16em] text-teal-100"><Sparkles className="h-3 w-3" />Structured screening</span><span className="inline-flex items-center gap-1.5 rounded-full border border-white/10 bg-white/[0.05] px-2.5 py-1 text-[10px] text-slate-300"><Clock3 className="h-3 w-3" />About 15–20 minutes</span></div><h1 className="mt-4 text-3xl font-bold tracking-[-0.04em] sm:text-[34px]">Hi {candidate.name.split(" ")[0]}, let’s talk about your experience.</h1><p className="mt-3 max-w-2xl text-sm leading-6 text-teal-50/80">This interview is for the <strong className="text-white">{position.title}</strong> position. Answer naturally—there are no trick questions, and a human recruiter makes every decision.</p></div></div>
        <div className="p-6 sm:p-8"><div className="grid gap-3 sm:grid-cols-2"><div className="rounded-xl border border-slate-200 bg-slate-50/70 p-4"><p className="flex items-center gap-2 text-xs font-bold text-slate-800"><ShieldCheck className="h-4 w-4 text-teal-600" />Human-reviewed</p><p className="mt-2 text-xs leading-5 text-slate-500">ScreenIT prepares job-related evidence; it does not make the hiring decision.</p></div><div className="rounded-xl border border-slate-200 bg-slate-50/70 p-4"><p className="flex items-center gap-2 text-xs font-bold text-slate-800"><Volume2 className="h-4 w-4 text-teal-600" />Your preferred format</p><p className="mt-2 text-xs leading-5 text-slate-500">Use voice or text. Voice recordings are not retained by default.</p></div></div>
          <label className={`mt-5 flex cursor-pointer items-start gap-3 rounded-xl border p-4 transition ${consented ? "border-teal-300 bg-teal-50/70 ring-2 ring-teal-100" : "border-slate-200 bg-white hover:border-teal-200"}`}><input type="checkbox" checked={consented} onChange={(event) => setConsented(event.target.checked)} className="mt-0.5 h-4 w-4 accent-teal-700" /><span className="text-sm leading-5 text-slate-700">I consent to AI-assisted transcription and analysis of my job-related answers for this screening. I understand a human will review the report.</span></label>
          ...
          <button disabled={!consented} onClick={begin} className="mt-5 flex h-12 w-full items-center justify-center gap-2 rounded-xl bg-gradient-to-r from-teal-700 to-emerald-700 text-sm font-bold text-white shadow-lg shadow-teal-900/15 transition hover:from-teal-800 hover:to-emerald-800 disabled:cursor-not-allowed disabled:opacity-40"><FileCheck2 className="h-4 w-4" />Begin interview</button>
        </div>
      </section>}
