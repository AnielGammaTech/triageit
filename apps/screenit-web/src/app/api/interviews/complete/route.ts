...
type GeneratedReport = Pick<CandidateReport, "summary" | "evidence" | "clarifications" | "roleAlignment" | "fitRationale">;
...
async function generateReport(requirements: readonly string[], transcript: string): Promise<GeneratedReport> {
  ...
  try {
    ...
    return { summary: "The structured interview is complete and ready for human review.", evidence: fallback, clarifications: ["Review the transcript before deciding next steps."], roleAlignment: "insufficient_evidence", fitRationale: "The AI report could not be completed, so a recruiter must review the transcript directly." };
  }
}
...
export async function POST(request: NextRequest) {
  ...
  const [reportResult, candidateResult, interviewResult] = await Promise.all([
    supabase.from("screenit_reports").upsert({ id: report.id, candidate_id: report.candidateId, summary: report.summary, requirement_evidence: report.evidence, clarifications: report.clarifications, recommendation: report.recommendation, role_alignment: report.roleAlignment, fit_rationale: report.fitRationale, generated_at: report.generatedAt }, { onConflict: "candidate_id" }),
