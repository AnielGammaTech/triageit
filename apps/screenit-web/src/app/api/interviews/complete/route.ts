...
type GeneratedReport = Pick<CandidateReport, "summary" | "evidence" | "clarifications" | "roleAlignment" | "fitRationale" | "statedMotivation" | "conversationSignals">;
...
async function generateReport(requirements: readonly string[], transcript: string): Promise<GeneratedReport> {
  ...
  if (!process.env.OPENAI_API_KEY || !transcript.trim()) return { summary: "The structured interview is complete and ready for human review.", evidence: fallback, clarifications: requirements.slice(0, 2).map((item) => `Confirm evidence for: ${item}`), roleAlignment: "insufficient_evidence", fitRationale: "There is not enough explicit interview evidence to assess alignment to the role requirements.", statedMotivation: "Not discussed.", conversationSignals: [] };
  try {
    const response = await fetch("https://api.openai.com/v1/responses", { method: "POST", headers: { Authorization: `Bearer ${process.env.OPENAI_API_KEY}`, "Content-Type": "application/json" }, body: JSON.stringify({
      ...
      store: false,
      input: `Create a structured recruiter evidence and role-alignment report. Use only explicit job-related statements in the transcript. Compare the evidence to each stated requirement. "Strong alignment" means the explicit interview evidence demonstrates most core requirements; it is not a hiring decision. Capture the candidate's stated motivation or interest only from their words and concrete examples in statedMotivation. In conversationSignals, provide at most five job-relevant, evidence-backed signals such as ownership, clarity of explanation, customer awareness, documentation habits, appropriate help-seeking, learning behavior, or explicitly stated interest. Every signal must cite something the candidate said or a concrete example they gave. Do not infer a trait from how they sounded. Never infer excitement, enthusiasm, emotion, personality, or engagement from voice, tone, accent, speed, pauses, or speaking style; use "Not discussed" when the candidate did not state motivation. If the transcript contains an explicit refusal to answer, repeated off-topic redirection, or clearly abusive/threatening language after a stated boundary, document the exact observable conduct neutrally in the summary or clarifications without diagnosing personality or intent. Never infer protected traits, personality, emotion, accent, honesty, or culture fit, and never issue a hire/reject recommendation. Requirements:\n${requirements.map((item) => `- ${item}`).join("\n")}\n\nTranscript:\n${transcript}`,
      text: { format: { type: "json_schema", name: "candidate_evidence_report", strict: true, schema: { type: "object", additionalProperties: false, required: ["summary", "evidence", "clarifications", "roleAlignment", "fitRationale", "statedMotivation", "conversationSignals"], properties: { summary: { type: "string" }, evidence: { type: "array", items: { type: "object", additionalProperties: false, required: ["requirement", "level", "evidence"], properties: { requirement: { type: "string" }, level: { type: "string", enum: ["demonstrated", "partial", "unclear", "not_demonstrated"] }, evidence: { type: "string" } } } }, clarifications: { type: "array", items: { type: "string" } }, roleAlignment: { type: "string", enum: ["strong_alignment", "partial_alignment", "limited_alignment", "insufficient_evidence"] }, fitRationale: { type: "string" }, statedMotivation: { type: "string" }, conversationSignals: { type: "array", maxItems: 5, items: { type: "object", additionalProperties: false, required: ["signal", "evidence"], properties: { signal: { type: "string" }, evidence: { type: "string" } } } } } } } },
    }) });
    ...
    return JSON.parse(outputText) as GeneratedReport;
  } catch (error) {
    console.error("[ScreenIT] Evidence report generation failed", error);
    return { summary: "The structured interview is complete and ready for human review.", evidence: fallback, clarifications: ["Review the transcript before deciding next steps."], roleAlignment: "insufficient_evidence", fitRationale: "The AI report could not be completed, so a recruiter must review the transcript directly.", statedMotivation: "Not discussed.", conversationSignals: [] };
  }
}
